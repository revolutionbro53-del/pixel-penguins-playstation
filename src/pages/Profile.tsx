...
const activityFeed = [
  {
    user: "GhostBlade42",
    action: "earned Platinum in God of War",
    time: "2h ago",
    reaction: { clap: 12, fire: 8, comment: 3 },
    seed: "ghostblade",
  },
  {
    user: "NightOwl_PS",
    action: "hit Level 38",
    time: "4h ago",
    reaction: { clap: 5, fire: 2, comment: 1 },
    seed: "nightowl",
  },
  {
    user: "ArcticFox99",
    action: "completed Spider-Man 2 100%",
    time: "6h ago",
    reaction: { clap: 22, fire: 14, comment: 7 },
    seed: "arctic",
  },
  {
    user: "GhostBlade42",
    action: 'earned Gold: "Speed Runner" in GT7',
    time: "Yesterday",
    reaction: { clap: 9, fire: 5, comment: 2 },
    seed: "ghostblade",
  },
  {
    user: "ShadowRunner",
    action: "started playing Bloodborne",
    time: "2d ago",
    reaction: { clap: 3, fire: 6, comment: 0 },
    seed: "shadow",
  },
];
  ...
  wishlist: [
    {
      ...
      progress: 0,
      image:
        "https://images.unsplash.com/photo-1593305841991-05c297ba4575?w=200&q=80",
    },
    {
      title: "Wolverine",
      progress: 0,
      image:
        "https://images.unsplash.com/photo-1606144042614-b2417e99c4e3?w=200&q=80",
    },
  ],
};
...
function CountUp({ target }: { target: number }) {
  ...
  return (
    <span ref={ref} className="count-up-animate">
      {val.toLocaleString()}
    </span>
  );
}
...
export default function Profile() {
  ...
  const filteredTrophies =
    trophyFilter === "All"
      ? trophyData
      : trophyData.filter((t) => t.type === trophyFilter.toLowerCase());
    ...
    <div className="min-h-screen pb-20 md:pb-8 md:pt-16">
        ...
        <div className="relative w-full px-4 md:px-8 lg:px-20 pb-8 pt-24 md:pt-16">
          <div className="flex flex-col md:flex-row items-start md:items-end gap-5">
            <div className="relative">
              <div
                className="w-24 h-24 rounded-2xl overflow-hidden border-2 border-ps-neon"
                style={{ boxShadow: "0 0 20px hsl(200 100% 50% / 0.5)" }}
              >
                <img
                  src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user.avatarSeed}`}
                  className="w-full h-full bg-ps-surface-2"
                  alt="Avatar"
                />
              </div>
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 flex-wrap mb-1">
                <h1 className="font-rajdhani font-bold text-4xl">
                  {user.username}
                </h1>
                <span
                  className="bg-ps-blue/20 border border-ps-blue text-ps-neon text-xs font-rajdhani px-3 py-1 rounded-full"
                  style={{ boxShadow: "0 0 8px hsl(200 100% 50% / 0.3)" }}
                >
                  Level {user.level}
                </span>
              </div>
              <p className="text-ps-secondary font-inter text-sm mb-1">
                @{user.psnId}
              </p>
              <p className="text-ps-secondary font-inter text-sm mb-3 max-w-md">
                {user.bio}
              </p>
              <div className="flex items-center gap-3">
                <div className="flex-1 max-w-xs">
                  <div className="flex justify-between text-[11px] text-ps-secondary mb-1">
                    <span>XP</span>
                    <span>
                      {user.xpCurrent.toLocaleString()} /{" "}
                      {user.xpNext.toLocaleString()}
                    </span>
                  </div>
                  <div className="w-full h-2 bg-ps-border rounded-full">
                    <div
                      className="h-full bg-ps-neon rounded-full xp-bar"
                      style={{ width: `${xpPct}%` }}
                    />
                  </div>
                </div>
      ...
      <div className="px-4 md:px-8 lg:px-20 py-8 space-y-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <motion.div
              key={stat.label}
              className="ps-card p-5 text-center"
              whileHover={{ y: -3 }}
            >
              <p className="font-rajdhani font-bold text-3xl text-ps-neon">
                <CountUp target={stat.value} />
              </p>
              <p className="text-ps-secondary text-sm font-inter mt-1">
                {stat.label}
              </p>
            </motion.div>
          ))}
            ...
            <AnimatePresence mode="popLayout">
              {filteredTrophies.map((t) => {
                ...
                return (
                  <motion.div
                    key={t.name}
                    className={`ps-glass rounded-xl p-4 border ${trophyBorder[t.type]}`}
                    layout
                  ...
                  >
                    ...
                    <p
                      className={`font-rajdhani font-bold text-base ${trophyColor[t.type]}`}
                    >
                      {t.name}
                    </p>
                    <p className="text-ps-secondary text-xs">{t.game}</p>
                    <p className="text-ps-secondary text-xs mt-1">{t.date}</p>
                    <p className="text-ps-secondary text-[11px] mt-1">
                      Earned by {t.rarity} of players
                    </p>
                  </motion.div>
                );
              })}
        ...
        {/* Game Library */}
        <section>
          <h2 className="font-rajdhani font-bold text-2xl mb-4">
            Game Library
          </h2>
                    ...
                    <div className="relative rounded-xl overflow-hidden mb-2">
                      ...
                      {g.progress > 0 && (
                        <div className="absolute bottom-0 left-0 right-0 h-1 bg-ps-border">
                          <div
                            className="h-full bg-ps-neon"
                            style={{ width: `${g.progress}%` }}
                          />
                        </div>
                      )}
                    </div>
                    <p className="font-rajdhani text-xs font-semibold truncate">
                      {g.title}
                    </p>
                    {g.progress > 0 && (
        ...
        <section>
          ...
          <div className="space-y-3">
            {activityFeed.map((item, i) => (
              <motion.div
                key={i}
                className="ps-glass rounded-xl p-4 flex gap-3"
              ...
              >
                <img
                  ...
                  className="w-10 h-10 rounded-full bg-ps-surface-2 flex-shrink-0"
                  alt={item.user}
                />
                <div className="flex-1">
                  ...
                  <p className="text-ps-secondary text-[11px] mt-0.5">
                    {item.time}
                  </p>
                  <div className="flex gap-3 mt-2">
                    {[
                      { icon: "👏", count: item.reaction.clap },
                      { icon: "🔥", count: item.reaction.fire },
                      { icon: "💬", count: item.reaction.comment },
                    ].map((r) => (
                      <button
                        key={r.icon}
                        className="flex items-center gap-1 text-ps-secondary hover:text-foreground transition-colors text-xs"
                      >
                        <span>{r.icon}</span>
                        <span>{r.count}</span>
                      </button>
      ...
      >
        <div className="space-y-4">
          <div>
            <label className="text-ps-secondary text-sm font-inter block mb-1">
              Display Name
            </label>
            <input
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full ps-glass border border-ps-border rounded-xl px-3 py-2 text-foreground text-sm focus:border-ps-neon focus:outline-none resize-none"
            />
          </div>
          <div>
            <label className="text-ps-secondary text-sm font-inter block mb-1">
            ...
            <select
              ...
              onChange={(e) =>
                setEditData((d) => ({ ...d, favoriteGenre: e.target.value }))
              }
            ...
            >
              {["Action RPG", "FPS", "Racing", "Horror", "Indie", "Sports"].map(
                (g) => (
          ...
          <div>
            <label className="text-ps-secondary text-sm font-inter block mb-2">
              Avatar Style
            </label>
            <div className="grid grid-cols-6 gap-2">
              {avatarSeeds.map((seed) => (
                <div
                  key={seed}
                  onClick={() =>
                    setEditData((d) => ({ ...d, avatarSeed: seed }))
                  }
                  ...
                  style={
                    editData.avatarSeed === seed
                      ? { boxShadow: "0 0 8px hsl(200 100% 50% / 0.5)" }
                      : {}
                  }
                >
                  <img
